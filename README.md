...
| Google "unsupported in your country" but the server's own browser opens the site fine | The exit IP is **not** the problem — a browser on the server proves the egress region is allowed. The client leaks a path you don't control. Three concrete causes, all fixed by mirroring xbox: **(1)** malformed `HTTPS/SVCB` so `alpn=h2` never applies (SvcParam keys must be RFC 9460 `alpn=1, ipv4hint=4, ipv6hint=5`; an off‑by‑one makes Chrome ignore it → QUIC to the real edge). Verify with `dig @<ip> gemini.google.com HTTPS +noall +answer` — it must *decode*, not print `Got bad packet: FORMERR`. **(2)** synthesizing `AAAA` to the server's own IPv6: if Google Geo‑tags that IPv6 differently (here the box IPv4 reads EU but `intl/ru` appears on the homepage — a red‑herring soft signal, not the gate), return **A/IPv4 only** (no `AAAA`) like xbox. **(3)** the TLS proxy resolving the upstream hostname itself and picking Google's IPv6 → egress on the RU‑tagged IPv6 leg → set `resolver … ipv6=off` so every leg leaves via the proven EU IPv4. After all three, the phone reaches the EU proxy over HTTP/2 with no IPv6/QUIC leak. |
