...
| Server IP is itself geolocated **RU** by Google | Even 100% EU‑*routed* traffic is then seen as RU and geo‑gated. Check what **Google** (not ipinfo) thinks of the exit: `curl -4 https://www.google.com/ \| grep -o intl/[a-z]*/` and the same with `-6`. If it says `intl/ru/` on both stacks, this box's IPs are RU‑tagged — no DoT/DoH/SVCB/QUIC change fixes that. Free **Cloudflare WARP** from the same box was also `intl/ru` here. Only an exit whose IP Google geolocates to a supported region (a clean EU VPS / residential‑EU proxy, chained via WireGuard + policy route for Google's ranges) will unblock Gemini/other IP‑geo services. |
