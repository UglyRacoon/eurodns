...
cp -f "$SCRIPT_DIR/resolver/smartdns-resolver.py" /opt/smartdns/smartdns-resolver.py
chmod 755 /opt/smartdns/build.sh; chmod 644 /opt/smartdns/smartdns_backend.py /opt/smartdns/smartdns-resolver.py
...
# env.conf consumed by build.sh, the resolver, and the systemd units.
# NOTE: public :53 is served by eurodns-resolver (A/AAAA/SVCB for geo domains);
# dnsmasq is only a local forwarder on 127.0.0.1:5353 for everything else.
...
# start forwarder + resolver + nginx. Resolver binds the public :53; dnsmasq is a
# loopback forwarder on :5353, so they never clash for port 53.
install -D -m 644 "$SCRIPT_DIR/systemd/eurodns-resolver.service" /etc/systemd/system/eurodns-resolver.service
systemctl daemon-reload
systemctl enable dnsmasq nginx eurodns-resolver >/dev/null 2>&1 || true
systemctl restart dnsmasq || true
# make sure nothing else grabbed :53 before the resolver does
fuser -k 53/udp 2>/dev/null || true; fuser -k 53/tcp 2>/dev/null || true; sleep 0.5
systemctl restart eurodns-resolver || warn "resolver failed to start (journalctl -u eurodns-resolver)"
